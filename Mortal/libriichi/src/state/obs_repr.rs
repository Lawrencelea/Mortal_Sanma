...
use crate::consts::{
    ...
    ACTION_RYUKYOKU, ACTION_SPACE, MAX_VERSION, obs_shape,
};
use crate::tile::Tile;
use crate::{tu8, tuz};
...
impl IntegerEncoder {
    ...
    fn encode(self, ctx: &mut ObsEncoderContext<'_>) {
        ...
        match ctx.version {
            ...
            // version 5 (sanma) uses the same encoding scheme as version 4
            4 | 5 => {
...
impl<'a> ObsEncoderContext<'a> {
    fn new(state: &'a PlayerState, version: u32, at_kan_select: bool) -> Self {
        ...
        // version 5 is sanma (3-player); all other versions are 4-player
        let num_players = if version == 5 { 3 } else { 4 };
        Self {
            state,
            arr,
            mask,
    ...
    fn encode_obs(mut self) -> (Array2<f32>, Array1<bool>) {
        ...
        for &score in &state.scores[..self.num_players] {
        ...
        self.idx += self.num_players;

        let n = state.kyoku as usize;
        if self.version == 1 {
            // for v1, this was a mistake, it actually only uses 3 channels.
            self.arr.fill_rows(self.idx, n, 1.);
        } else {
            self.arr.fill(self.idx + n, 1.);
        }
        self.idx += if self.version == 5 { 3 } else { 4 };
        ...
        if matches!(self.version, 2 | 3 | 4 | 5) {
            let seats_per_wind = if self.version == 5 { 3 } else { 4 };
            let n = (state.bakaze.as_u8() - tu8!(E)).min(1) * seats_per_wind + state.kyoku;
            let cap = if self.version == 5 { 5 } else { 7 };
            IntegerEncoder::new(n as usize, cap)
                .rescale(true)
                .encode(&mut self);
        }
        ...
        let max_kawa_len = state.kawa[..self.num_players]
            .iter()
            .map(|k| k.len())
            .max()
            .unwrap();
        ...
        for player_kawa in &state.kawa[1..self.num_players] {
            player_kawa
                .iter()
                .take(6)
        ...
        let v = state.tiles_left as f32 / 55.;
        self.arr.fill(self.idx, v);
        self.idx += 1;

        for &count in &state.doras_owned[..self.num_players] {
        ...
        for player_kawa_overview in &state.kawa_overview[..self.num_players] {
            self.encode_tile_set(player_kawa_overview.iter().copied());
        }

        for player_fuuro in &state.fuuro_overview[..self.num_players] {
        ...
        for player_ankan in &state.ankan_overview[..self.num_players] {
        ...
        if matches!(self.version, 2 | 3 | 4 | 5) {
            ...
            for &player_last_tedashi in &state.last_tedashis[1..self.num_players] {
            ...
            for &player_riichi_sutehai in &state.riichi_sutehais[1..self.num_players] {
        ...
        state.riichi_declared[1..self.num_players]
            .iter()
            .enumerate()
            .filter(|&(_, &b)| b)
            .for_each(|(i, _)| self.arr.fill(self.idx + i, 1.));
        self.idx += num_opponents;
        state.riichi_accepted[1..self.num_players]
            .iter()
            .enumerate()
            .filter(|&(_, &b)| b)
            .for_each(|(i, _)| self.arr.fill(self.idx + i, 1.));
        ...
        if cans.can_ryukyoku {
            self.arr.fill(self.idx, 1.);
            if !self.at_kan_select {
                self.mask[ACTION_RYUKYOKU] = true;
            }
        }
        self.idx += 1;
