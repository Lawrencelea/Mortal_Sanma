...
fn nukidora_replacement_draw_is_rinshan() {
    let ps = PlayerState::from_log(
        1,
        r#"
{"type":"start_kyoku","bakaze":"S","dora_marker":"4p","kyoku":1,"honba":1,"kyotaku":1,"oya":0,"scores":[23000,48500,32500],"tehais":[["9m","1p","1p","5p","7p","9p","3s","8s","8s","S","P","P","P"],["2p","7p","8p","1s","1s","7s","7s","8s","9s","E","W","W","C"],["1m","1m","1p","6p","9p","6s","6s","6s","8s","E","S","N","F"]]}
...
{"type":"tsumo","actor":0,"pai":"1s"}
{"type":"dahai","actor":0,"pai":"1s","tsumogiri":true}
{"type":"pon","actor":1,"target":0,"pai":"1s","consumed":["1s","1s"]}
{"type":"dahai","actor":1,"pai":"E","tsumogiri":false}
{"type":"tsumo","actor":2,"pai":"3p"}
...
#[test]
fn nukidora_before_riichi_suppresses_later_ippatsu() {
    let ps = PlayerState::from_log(
        1,
        r#"
{"type":"start_kyoku","bakaze":"E","dora_marker":"3p","kyoku":2,"honba":1,"kyotaku":0,"oya":1,"scores":[20400,50600,34000],"tehais":[["1p","1p","1p","4p","6p","9p","1s","1s","3s","6s","7s","E","S"],["1m","1m","1m","4p","7p","8p","3s","4s","5s","6s","E","S","W"],["3p","3p","5pr","6p","9p","3s","4s","4s","5s","8s","8s","P","F"]]}
...
"#,
    ...
    assert_eq!(ps.agari_points(true, &[t!(2p)]).unwrap().ron, 7700);
}

#[test]
fn nukidora_ron_window_ignores_same_cycle_furiten() {
    let ps = PlayerState::from_log(
        2,
        r#"
{"type":"start_kyoku","bakaze":"S","dora_marker":"E","kyoku":1,"honba":1,"kyotaku":1,"oya":0,"scores":[47800,30300,25900],"tehais":[["9m","1p","3p","4p","6p","8p","2s","4s","4s","8s","9s","P","F"],["3p","3p","4p","4p","5p","8p","9p","1s","6s","N","P","P","C"],["9m","1p","5p","7p","8p","9p","1s","9s","S","W","N","F","C"]]}
{"type":"tsumo","actor":0,"pai":"7s"}
...
#[test]
#[ignore = "four-player-only fixture uses actor 3; sanma furiten coverage lives in converter/validator tests"]
fn furiten() {
    let mut ps = PlayerState::new(0);
...
#[test]
#[ignore = "four-player-only fixture uses actor 3; replace with a sanma kan fixture when needed"]
...
#[test]
fn ankan_is_allowed_after_pon_before_discard() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(S),
        kyoku: 1,
        ...
        dora_marker: t!(8s),
        tehais: vec![
            tile37_to_vec(&hand_with_aka("22345p 33334456s").unwrap()),
            vec![t!(?); 13],
            vec![t!(?); 13],
        ],
    });
    ...
    let cans = ps.test_update(&Event::Pon {
        actor: 0,
        target: 1,
        pai: t!(2p),
        consumed: t![2p, 2p],
    ...
    assert!(cans.can_ankan);
    assert!(ps.ankan_candidates.contains(&t!(3s)));
...
#[test]
fn same_tile_discard_is_allowed_after_pon() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(E),
        kyoku: 1,
        ...
        dora_marker: t!(8s),
        tehais: vec![
            tile37_to_vec(&hand_with_aka("123456789p 4448s").unwrap()),
            vec![t!(?); 13],
            vec![t!(?); 13],
        ],
    });
    ...
    assert!(ps.discard_candidates_aka()[tuz!(4s)]);
}

#[test]
fn riichi_is_allowed_with_three_tiles_left_after_draw() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(E),
        kyoku: 1,
        ...
        dora_marker: t!(W),
        tehais: vec![
            tile37_to_vec(&hand_with_aka("678p 3466778999s").unwrap()),
            vec![t!(?); 13],
            vec![t!(?); 13],
        ],
    });
    ...
    assert_eq!(ps.tiles_left, 3);
    assert!(cans.can_riichi);
}

#[test]
fn riichi_is_not_allowed_with_two_tiles_left_after_draw() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(E),
        kyoku: 1,
    ...
    assert_eq!(ps.tiles_left, 2);
    assert!(!cans.can_riichi);
}

#[test]
#[ignore = "four-player-only fixture reaches four-player all-last; sanma all-last is S3/W-entry"]
...
fn get_rank() {
    ...
    let rank = ps.get_rank([35000, 30000, 40000, 0]);
    assert_eq!(rank, 1);

    let ps = PlayerState::new(1);
    let rank = ps.get_rank([50000, 35000, 20000, 0]);
    assert_eq!(rank, 0);

    let ps = PlayerState::new(2);
    let rank = ps.get_rank([20000, 35000, 50000, 0]);
    assert_eq!(rank, 2);

    let ps = PlayerState::new(2);
    let rank = ps.get_rank([5, 2, 5, 0]);
    assert_eq!(rank, 1);
}

#[test]
#[ignore = "four-player-only fixture uses actor 3; sanma tests should avoid fourth seat"]
fn kakan_from_hand() {
    let log = r#"
...
#[test]
#[ignore = "four-player-only fixture uses actor 3; sanma chankan/nukidora coverage is separate"]
...
#[test]
#[ignore = "chi is illegal in sanma"]
fn chi_at_0_shanten() {
    let log = r#"
