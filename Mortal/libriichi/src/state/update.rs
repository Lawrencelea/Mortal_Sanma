...
impl PlayerState {
    ...
    fn update_inner(
        ...
        if let Some(tile) = self.to_mark_same_cycle_furiten.take()
            && !event_claims_tile(event, tile)
        {
            self.same_cycle_furiten = !self.at_furiten;
        ...
        match event {
            ...
            Event::Chi { .. } => anyhow::bail!("chi is not legal in Tenhou sanma"),
    ...
    fn start_kyoku(
        ...
        let seat_offset = (self.player_id + 3 - oya) % 3;
        self.jikaze = match seat_offset {
            0 => must_tile!(tu8!(E)),
            1 => must_tile!(tu8!(S)),
            2 => must_tile!(tu8!(W)),
            _ => unreachable!(),
        ...
        for rel in 0..3 {
            ...
            if let Some(score) = scores.get(abs) {
                self.scores[rel] = *score;
            }
        }
        ...
        // Tenhou sanma has 108 tiles. After the initial 13-tile hands and
        // the 14-tile dead wall, there are 55 live-wall draws.
        self.tiles_left = 55;
        self.at_turn = 0;
        ...
        let Some(tehai) = tehais.get(self.player_id as usize) else {
            return Ok(());
        };
        for &t in tehai {
            self.witness_tile(t)?;
    ...
    fn nukidora(&mut self, actor: u8, pai: Tile) -> Result<()> {
        ...
        self.at_ippatsu = false;
        // Tenhou sanma treats the replacement draw after kita/nuki-dora as a
        // rinshan draw. This matters for open hands whose only yaku is
        // 嶺上開花 after extracting north.
    ...
    fn chi(&mut self, actor: u8, pai: Tile, consumed: [Tile; 2]) -> Result<()> {
        ...
        self.update_shanten();
        self.update_shanten_discards();
        self.update_kan_candidates_after_call();
    ...
    fn update_nukidora_candidate(&mut self) {
        if self.tiles_left > 0 && self.tehai[tuz!(N)] > 0 {
            self.last_cans.can_nukidora = true;
        }
    }

    fn update_kan_candidates_after_call(&mut self) {
        // Tenhou allows a concealed/additional kan before the discard that
        // follows a chi/pon call. This is a discard-turn, so riichi/tsumo
        // checks do not apply, but the kan limit and haitei restriction do.
        if self.tiles_left == 0 || self.kans_on_board >= 4 {
            return;
        }
    ...
    fn can_ron_on_special_tile(&self, pai: Tile) -> bool {
        // Tenhou sanma treats nuki-dora ron like chankan: it opens a special
        // ron window on a declaration, not a normal discard. Same-cycle
    ...
    pub(super) fn get_rank(&self, scores_rel: [i32; 4]) -> u8 {
...
fn event_claims_tile(event: &Event, tile: Tile) -> bool {
    match event {
        Event::Chi { pai, .. } | Event::Pon { pai, .. } | Event::Daiminkan { pai, .. } => {
            pai.deaka() == tile.deaka()
        }
        _ => false,
    }
}
