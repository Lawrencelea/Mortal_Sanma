...
impl Gameplay {
    fn load_events_by_player(
        ...
        let num_players = grp.num_players;
        ...
        let opponent_states = (1..num_players as u8)
            .map(|i| PlayerState::new((player_id + i) % num_players as u8))
            .collect::<Vec<_>>();
    ...
    fn extend_from_event_window(
        ...
        let label_opt = match *next {
            ...
            Event::Chi { .. } => anyhow::bail!("sanma gameplay loader does not accept chi events"),
    ...
    fn add_entry(&mut self, ctx: &LoaderContext<'_>, at_kan_select: bool, label: usize) {
        ...
        // Discards, riichi declarations, and nuki declarations advance the
        // live-wall/replacement-draw timeline for the acting player.
        self.apply_gamma
            .push(label <= ACTION_RIICHI || label == ACTION_NUKIDORA);
