use crate::consts::{GRP_SANMA_SIZE, GRP_SIZE};
...
impl Grp {
    ...
    fn load_log_files_py(filenames: Vec<PyBackedStr>) -> Result<Vec<Self>> {
    ...
    pub const fn take_num_players(&self) -> usize {
        self.num_players
    }
}
...
impl Grp {
    ...
    pub fn load_log_files<V, S>(filenames: V) -> Result<Vec<Self>>
    where
        V: IntoParallelIterator<Item = S>,
        S: AsRef<str>,
    {
        filenames
            ...
            .map(|f| {
                let filename = f.as_ref();
                let inner = || {
                    let raw = io::read_to_string(File::open(filename)?)?;
                    Self::load_log(&raw)
                };
                inner().with_context(|| format!("error when reading {filename}"))
            })
    ...
    pub fn load_events(events: &[Event]) -> Result<Self> {
        // Detect number of players from StartGame event.
        let num_players = events
            .iter()
            .find_map(|ev| {
                if let Event::StartGame { names, .. } = ev {
                    Some(names.len())
                } else {
                    None
                }
            ...
            .unwrap_or(3);
        let grp_size = if num_players == 3 {
            GRP_SANMA_SIZE
        } else {
            GRP_SIZE
        };
        // Expected total score for the game type.
        let expected_total: i32 = if num_players == 3 { 105_000 } else { 100_000 };
        ...
        for ev in events.iter().rev() {
            match ev {
                ...
                Event::StartKyoku {
                    ...
                    if rank_by_player_opt.is_none() {
                        final_scores.fill(0);
                        for (dst, src) in final_scores.iter_mut().zip(scores) {
                            *dst = *src;
                        }
                        ...
                        if sum < expected_total {
                            final_scores[rk.player_by_rank[0] as usize] += expected_total - sum;
                        }
