...
pub struct BatchGame {
    /// 6 for Tenhou sanma hanchan (E1-E3, S1-S3).
    pub length: u8,
...
struct Game {
    ...
    kyoku_started: bool,
    ended: bool,
    /// Used in 西入 where the oya and another player get to 40000 at the same
    /// time, but the game continues because oya is not the top.
    ///
    /// Tenhou sanma uses a 40000-point target for South 3 / West-entry
    /// sudden death, with dealer renchan taking priority.
    in_renchan: bool,
}
...
impl Game {
    ...
    fn poll(&mut self, agents: &mut [Box<dyn BatchAgent>]) -> Result<()> {
        ...
        if !self.kyoku_started {
            // Tenhou sanma enters 西入 after South 3 when all three players
            // are still below 40000. Once in sudden death, the next hand
            // boundary with anyone at/above 40000 ends the game unless dealer
            // renchan takes priority.
            if self.kyoku >= self.length + 3
                || self.kyoku >= self.length
                    && !self.in_renchan
                    && self.scores.iter().any(|&s| s >= 40000)
            {
                self.ended = true;
                return Ok(());
            }
        ...
        match poll {
            ...
            Poll::End => {
                ...
                // 1. can renchan
                // 2. is at all-last
                // 3. oya has at least 40000
                // 4. oya is the top
                let oya = kyoku_result.kyoku as usize % 3;
                if kyoku_result.kyoku >= self.length - 1 && self.scores[oya] >= 40000 {
...
mod test {
    ...
    fn tsumogiri() {
        ...
        let mut agents = [Box::new(Tsumogiri::new_batched(&[0, 1, 2, 0, 1, 2]).unwrap()) as _];
        let indexes = &[
            [
                Index {
                    agent_idx: 0,
