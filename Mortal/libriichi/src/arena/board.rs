...
pub struct Board {
    ...
    pub kyotaku: u8,
    /// Tenhou sanma starts from [35000; 3].
    pub scores: [i32; 3],
...
impl Board {
    pub fn init_from_seed(&mut self, game_seed: (u64, u64)) {
        ...
        idx += 5;
        self.yama = seq[idx..idx + 55].to_vec();
        idx += 55;
...
impl BoardState {
    ...
    const fn update_nagashi_mangan_and_four_wind(&mut self, ev: &Event) {
        match *ev {
            ...
            Event::Pon { target, .. } | Event::Daiminkan { target, .. } => {
    ...
    fn handle_hora(
        ...
        if let Some(pao_target) = self.paos[single_actor as usize] {
            ...
            deltas[pao_target as usize] = -point.ron - honba_left * 200;
        } else {
        ...
        let tsumo_total = if single_actor == self.oya {
            point.tsumo_ko * 2
        } else {
            point.tsumo_ko + point.tsumo_oya
        };
        deltas[single_actor as usize] = tsumo_total + kyotaku_point + honba_left * 200;
    ...
    fn step(&mut self, reactions: &[EventExt; 3]) -> Result<Poll> {
        ...
        match ev.event {
            ...
            Event::Nukidora { actor, .. } => {
                ...
                // After nukidora the actor draws a regular tsumo from yama,
                // not from rinshan (confirmed by real Tenhou sanma game logs).
                self.tsumo_actor = actor;
            }
    ...
    pub fn encode_oracle_obs(&self, perspective: u8, version: u32) -> Array2<f32> {
        ...
        idx += (54 - self.tiles_left as usize) * 2;
