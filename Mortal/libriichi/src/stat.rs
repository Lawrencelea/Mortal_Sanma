...
impl Stat {
    ...
    pub fn from_game(events: &[Event], player_id: u8) -> Self {
        ...
        let num_players = events
            ...
            .unwrap_or(3);
        let expected_total = if num_players == 3 { 105_000 } else { 100_000 };
        let init_score = if num_players == 3 { 35_000 } else { 25_000 };
        ...
        events.iter().for_each(|ev| match ev {
            Event::StartKyoku { oya, scores, .. } => {
                stat.round += 1;
                cur_scores.fill(0);
                for (dst, src) in cur_scores.iter_mut().zip(scores) {
                    *dst = *src;
                }
        ...
        let rk = Rankings::new_n(&cur_scores, num_players);

        let sum: i32 = cur_scores[..num_players].iter().sum();
        if sum < expected_total {
            cur_scores[rk.player_by_rank[0] as usize] += expected_total - sum;
        }

        let final_score = cur_scores[player_id as usize];
        stat.point = final_score as i64 - init_score;
        if final_score < 0 {
            stat.tobi = 1;
        }
