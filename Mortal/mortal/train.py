def train():
    ...
    test_games = config.get('test_play', {}).get('games', 0)
    ...
    test_player = TestPlayer() if test_games > 0 else None
    best_perf = {
        'avg_rank': 4.,
        'avg_pt': -135.,
    }
    ...
    def train_epoch():
        ...
        def train_batch(obs, actions, masks, steps_to_done, kyoku_rewards, player_ranks):
            ...
            if steps % save_every == 0:
                ...
                if test_player is not None and steps % test_every == 0:
                    stat = test_player.test_play(test_games // 3, mortal, dqn, device)
                    mortal.train()
                    dqn.train()

                    display_pts = [int(p) for p in pts]
                    if len(display_pts) == 3:
                        display_pts.append(0)
                    ...
                    if version != 5:
                        rank_scalars['4th'] = stat.rank_4_rate
                    writer.add_scalars('test_play/ranking', rank_scalars, steps)
