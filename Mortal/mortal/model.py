...
from libriichi.consts import obs_shape, oracle_obs_shape, ACTION_SPACE, GRP_SIZE, GRP_SANMA_SIZE
...
class GRP(nn.Module):
    def __init__(self, hidden_size=64, num_layers=2, num_players=4):
        ...
        input_size = GRP_SANMA_SIZE if num_players == 3 else GRP_SIZE
        num_perms = 6 if num_players == 3 else 24  # 3! = 6, 4! = 24
        ...
        perms = torch.tensor(list(permutations(range(num_players))))
    ...
    def calc_matrix(self, logits: Tensor):
        ...
        matrix = torch.zeros(batch_size, n, n, dtype=probs.dtype)
