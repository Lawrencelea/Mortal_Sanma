...
class FileDatasetsIter(IterableDataset):
    ...
    def build_iter(self):
        ...
        self.grp = GRP(**config['grp']['network'], num_players=3)
    ...
    def populate_buffer(self, file_list):
        if file_list and file_list[0].endswith('.jsonl'):
            data = self.loader.load_log_files(file_list)
        else:
        ...
        for file in data:
            for game in file:
                ...
                rank_by_player = grp.take_rank_by_player()[:3]
                ...
                final_scores = grp.take_final_scores()[:3]
