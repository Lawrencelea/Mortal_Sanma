...
class GrpFileDatasetsIter(IterableDataset):
    ...
    def populate_buffer(self, start_idx):
        ...
        if file_list and file_list[0].endswith('.jsonl'):
            data = Grp.load_log_files(file_list)
        else:
...
def train():
    ...
    num_players = cfg.get('control', {}).get('num_players', 3)
    ...
    if path.exists(file_index):
        ...
        if not val_file_list:
            ...
            split = max(1, int(len(train_file_list) * val_ratio))
            val_file_list = train_file_list[:split]
            train_file_list = train_file_list[split:]
            logging.info(f'auto val split: {len(val_file_list):,} val / {len(train_file_list):,} train')
    ...
    for inputs, rank_by_players in train_data_loader:
        ...
        labels = grp.get_label(rank_by_players[:, :num_players])
