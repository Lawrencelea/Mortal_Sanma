        ...
        for (i, event) in events.iter().enumerate() {
            ...
            match event.clone() {
                Event::StartKyoku {
                    ...
                    kyoku_review.kyoku = (bakaze.as_u8() - tu8!(E)) * 3 + kk - 1;
                    kyoku_review.honba = honba;
                    kyoku_review.relative_scores = {
                        let mut scores_array = [0; 3];
                        for (dst, src) in scores_array.iter_mut().zip(scores) {
                            *dst = src;
                        }
                        scores_array
                    };
            ...
            let can_pon_or_daiminkan = masks[39] || masks[40];
            ...
            let at_opponent_kakan = matches!(event, Event::Kakan { .. } | Event::Nukidora { .. });
...
fn equal_ignore_aka_consumed(a: &Event, b: &Event) -> bool {
    match (a, b) {
        ...
        | (Event::Kakan { pai: l, .. }, Event::Kakan { pai: r, .. })
        | (Event::Nukidora { pai: l, .. }, Event::Nukidora { pai: r, .. }) => l == r,
...
fn to_label(ev: &Event) -> usize {
    match ev {
        ...
        Event::Nukidora { .. } => 38,
        Event::Pon { .. } => 39,
        Event::Daiminkan { .. } | Event::Ankan { .. } | Event::Kakan { .. } => 40,
        Event::Hora { .. } => 41,
        Event::Ryukyoku { .. } => 42,
        _ => 43,
    }
...
fn to_event(
    ...
    let event = match label {
        ...
        38 => Event::Nukidora { actor, pai: t!(N) },
        39 => {
        ...
        40 => {
            // Daiminkan / Ankan / Kakan
            let tile = last_tsumo_or_discard.context("missing last discard for Kan")?;
