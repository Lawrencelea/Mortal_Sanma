...
# Try the bundled platform extension first (Windows .pyd / Linux .so in this directory)
try:
    from .libriichi import *
    _inner = _sys.modules.get(f'{__name__}.libriichi') or _sys.modules.get('libriichi.libriichi')
except ImportError:
    # Fall back: search sys.path for the installed mortal libriichi .so
    import glob as _glob
    import importlib.util as _ilu

    _inner = None
    _self_dir = _os.path.abspath(_os.path.dirname(__file__))
    ...
    for _p in _sys.path:
        ...
        if _abs == _self_dir:
            continue
        for _m in (
            _glob.glob(_os.path.join(_abs, 'libriichi', 'libriichi*.so')) +
            _glob.glob(_os.path.join(_abs, 'libriichi', 'libriichi*.pyd'))
        ):
            try:
                _spec = _ilu.spec_from_file_location(f'{__name__}.libriichi', _m)
                if _spec and _spec.loader:
                    _inner = _ilu.module_from_spec(_spec)
                    _sys.modules[f'{__name__}.libriichi'] = _inner
    ...
    if _inner is None:
        raise ImportError("Could not find a libriichi compiled extension for this platform")

# Register submodules so 'from libriichi.consts import X' works
if _inner is not None:
    for _name in ('consts', 'state', 'dataset', 'arena', 'stat', 'mjai'):
        _sub = getattr(_inner, _name, None)
        if _sub is not None:
            _sys.modules[f'{__name__}.{_name}'] = _sub
    __doc__ = getattr(_inner, '__doc__', None)
    del _name, _sub
del _inner
